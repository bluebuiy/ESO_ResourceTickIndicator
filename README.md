...
What that means is if you stop blocking/sprinting frame-perfectly, you will fail to get the resource tick. This is because the server is recieving your input rtt/2 seconds later, which is past when it checks for the passive tick.  To compensate, the indicator's animation is shortened by rtt/2, measured from the last known good tick. (this is wrong, the animation needs to adjust based on current rtt, but works well enough in most cases and fixed the bad feeling it had initially.)
